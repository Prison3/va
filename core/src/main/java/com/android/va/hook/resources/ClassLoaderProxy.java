...
import com.android.va.utils.DexFileRecovery;
...
public class ClassLoaderProxy extends ClassInvocationStub {
    private static final String TAG = ClassLoaderProxy.class.getSimpleName();
    ...
    protected void inject(Object baseInvocation, Object proxyInvocation) {
        ...
        Logger.d(TAG, "ClassLoader proxy injected for DEX file corruption prevention");
    }
        ...
        try {
            ...
            if (hostContext != null) {
                ...
                if (hostClassLoader != null) {
                    ...
                    Logger.d(TAG, "Added host class loader as fallback");
                }
            }
            ...
            if (systemClassLoader != null) {
                ...
                Logger.d(TAG, "Added system class loader as fallback");
            }
            ...
            if (bootClassLoader != null) {
                ...
                Logger.d(TAG, "Added boot class loader as fallback");
            }
            
        } catch (Exception e) {
            Logger.w(TAG, "Failed to initialize fallback class loaders: " + e.getMessage());
        }
    }
    ...
    public static class LoadClass extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            if (sClassCache.containsKey(className)) {
                Logger.d(TAG, "ClassLoader: loadClass returning cached class: " + className);
                return sClassCache.get(className);
            }
            
            // Handle known problematic classes
            if (isProblematicClass(className)) {
                Logger.d(TAG, "ClassLoader: loadClass called for problematic class: " + className + ", returning null");
                return null;
            }
            ...
            try {
                ...
                if (result != null) {
                    ...
                    Logger.d(TAG, "ClassLoader: loadClass successful for: " + className);
                    return result;
                }
            } catch (Exception e) {
                Logger.w(TAG, "ClassLoader: loadClass failed for " + className + ", trying fallbacks: " + e.getMessage());
            }
            ...
            if (fallbackResult != null) {
                ...
                Logger.d(TAG, "ClassLoader: loadClass successful via fallback for: " + className);
                return fallbackResult;
            }
            
            // If all else fails, try to create a stub class or return null
            Logger.w(TAG, "ClassLoader: loadClass failed for " + className + ", returning null");
            return null;
        }
    }
    ...
    public static class FindClass extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            // Check cache first
            if (sClassCache.containsKey(className)) {
                Logger.d(TAG, "ClassLoader: findClass returning cached class: " + className);
                return sClassCache.get(className);
            }
            
            // Handle known problematic classes
            if (isProblematicClass(className)) {
                Logger.d(TAG, "ClassLoader: findClass called for problematic class: " + className + ", returning null");
                return null;
            }
            ...
            try {
                ...
                if (result != null) {
                    // Cache successful result
                    sClassCache.put(className, (Class<?>) result);
                    Logger.d(TAG, "ClassLoader: findClass successful for: " + className);
                    return result;
                }
            } catch (Exception e) {
                Logger.w(TAG, "ClassLoader: findClass failed for " + className + ", trying fallbacks: " + e.getMessage());
            }
            ...
            if (fallbackResult != null) {
                sClassCache.put(className, fallbackResult);
                Logger.d(TAG, "ClassLoader: findClass successful via fallback for: " + className);
                return fallbackResult;
            }
            
            // If all else fails, try to create a stub class or return null
            Logger.w(TAG, "ClassLoader: findClass failed for " + className + ", returning null");
            return null;
        }
    }
    ...
    public static class ForName extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            // Check cache first
            if (sClassCache.containsKey(className)) {
                Logger.d(TAG, "ClassLoader: forName returning cached class: " + className);
                return sClassCache.get(className);
            }
            
            // Handle known problematic classes
            if (isProblematicClass(className)) {
                Logger.d(TAG, "ClassLoader: forName called for problematic class: " + className + ", returning null");
                return null;
            }
            ...
            try {
                ...
                if (result != null) {
                    // Cache successful result
                    sClassCache.put(className, (Class<?>) result);
                    Logger.d(TAG, "ClassLoader: forName successful for: " + className);
                    return result;
                }
            } catch (Exception e) {
                Logger.w(TAG, "ClassLoader: forName failed for " + className + ", trying fallbacks: " + e.getMessage());
            }
            ...
            if (fallbackResult != null) {
                sClassCache.put(className, fallbackResult);
                Logger.d(TAG, "ClassLoader: forName successful via fallback for: " + className);
                return fallbackResult;
            }
            
            // If all else fails, try to create a stub class or return null
            Logger.w(TAG, "ClassLoader: forName failed for " + className + ", returning null");
            return null;
        }
    }
    ...
    public static class OpenDexFile extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            if (!isValidDexFile(sourceFile)) {
                Logger.w(TAG, "DexFile: Invalid DEX file detected: " + sourceFile + ", attempting recovery");
                
                // Try to recover or use fallback
                String recoveredFile = recoverDexFile(sourceFile);
                if (recoveredFile != null) {
                    args[0] = recoveredFile;
                    Logger.d(TAG, "DexFile: Using recovered DEX file: " + recoveredFile);
                } else {
                    Logger.e(TAG, "DexFile: Could not recover DEX file: " + sourceFile);
            ...
            try {
                ...
                Logger.w(TAG, "DexFile: openDexFile failed for " + sourceFile + ", returning null: " + e.getMessage());
                return null; // Return null to prevent crash
            }
        }
    }
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            try {
                ...
                if (errorMessage != null && errorMessage.contains("classes.dex") && errorMessage.contains("Entry not found")) {
                    Logger.w(TAG, "DexPathList: classes.dex entry not found, attempting recovery");
                    ...
                    if (corruptedFilePath != null) {
                        ...
                        if (recoveryResult.success) {
                            Logger.d(TAG, "DexPathList: Successfully recovered DEX file via " + recoveryResult.recoveryMethod);
                            ...
                            Logger.w(TAG, "DexPathList: DEX file recovery failed: " + recoveryResult.errorMessage);
                        }
                    }
                }
                
                Logger.w(TAG, "DexPathList: loadDexFile failed, returning null: " + e.getMessage());
                return null; // Return null to prevent crash
            }
        }
        ...
        private String extractCorruptedFilePathFromError(Exception e, Object who) {
            try {
                ...
                try {
                    ...
                    Logger.w(TAG, "Could not extract path via reflection: " + reflectionException.getMessage());
                }
                
            } catch (Exception ex) {
                Logger.w(TAG, "Error extracting corrupted file path: " + ex.getMessage());
            }
    ...
    private static Class<?> tryFallbackClassLoaders(String className) {
        for (ClassLoader fallbackLoader : sFallbackClassLoaders) {
            try {
                ...
                if (result != null) {
                    Logger.d(TAG, "Successfully loaded class " + className + " via fallback loader: " + fallbackLoader.getClass().getSimpleName());
                    return result;
                }
            } catch (Exception e) {
    ...
    private static boolean isValidDexFile(String sourceFile) {
        ...
        try {
            ...
            Logger.w(TAG, "Error validating DEX file " + sourceFile + ": " + e.getMessage());
            sDexFileCache.put(sourceFile, false);
            return false;
        }
    }
        ...
        try {
            ...
            if (hostContext != null) {
                ...
                if (appInfo != null && appInfo.sourceDir != null) {
                    Logger.d(TAG, "Using host app APK as DEX fallback: " + appInfo.sourceDir);
                    return appInfo.sourceDir;
                }
            }
            
        } catch (Exception e) {
            Logger.w(TAG, "Error recovering DEX file: " + e.getMessage());
        }
    ...
    private static String findAlternativeApkFile(String originalFile) {
        try {
                ...
                if (files != null) {
                    for (File file : files) {
                        if (file.getName().endsWith(".apk") && file.length() > 1000000) { // > 1MB
                            Logger.d(TAG, "Found alternative APK file: " + file.getAbsolutePath());
            ...
            Logger.w(TAG, "Error finding alternative APK file: " + e.getMessage());
        }
        return null;
    }
        ...
        try {
            ...
            if (hostContext != null) {
                ...
                if (appInfo != null && appInfo.sourceDir != null) {
                    ...
                    Logger.d(TAG, "Recovering DexPathList using host APK: " + appInfo.sourceDir);
                    return createFallbackDexPathList(who, appInfo.sourceDir);
                }
            }
        } catch (Exception e) {
            Logger.w(TAG, "Error recovering DexPathList: " + e.getMessage());
        }
        return null;
    }
    ...
    private static Object createFallbackDexPathList(Object who, String fallbackApk) {
        try {
            ...
            Logger.d(TAG, "Creating fallback DexPathList with: " + fallbackApk);
            return null; // Return null for now to prevent crash
        } catch (Exception e) {
            Logger.w(TAG, "Error creating fallback DexPathList: " + e.getMessage());
            return null;
        }
    }
    ...
    public static void clearClassCache() {
        sClassCache.clear();
        sDexFileCache.clear();
        Logger.d(TAG, "Class and DEX file caches cleared");
    }
