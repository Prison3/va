...
//        }
    ...
    JobService getJobService(int jobId) {
        synchronized (mJobRecords) {
            ...
            try {
                JobRecord record = VJobManager.get().queryJobRecord(VActivityThread.getAppProcessName(), jobId);
                record.mJobService = VActivityThread.currentActivityThread().createJobService(record.mServiceInfo);
