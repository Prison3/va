...
public class ActivityManagerService extends IActivityManagerService.Stub implements ISystemService {
    public static final String TAG = ActivityManagerService.class.getSimpleName();
    ...
    private final PackageManagerService mPms = PackageManagerService.get();
    private final BroadcastManager mBroadcastManager;

    public static ActivityManagerService get() {
        return sService;
    }
    ...
    public IBinder acquireContentProviderClient(ProviderInfo providerInfo) throws RemoteException {
        ...
        ProcessRecord processRecord = ProcessManagerService.get().startProcessLocked(providerInfo.packageName,
    ...
    public Intent sendBroadcast(Intent intent, String resolvedType, int userId) throws RemoteException {
        List<ResolveInfo> resolves = PackageManagerService.get().queryBroadcastReceivers(intent, GET_META_DATA, resolvedType, userId);

        for (ResolveInfo resolve : resolves) {
            ProcessRecord processRecord = ProcessManagerService.get().findProcessRecord(resolve.activityInfo.packageName, resolve.activityInfo.processName, userId);
            if (processRecord == null) {
                continue;
            }
            try {
    ...
    public RunningAppProcessInfo getRunningAppProcesses(String callerPackage, int userId) throws RemoteException {
        ...
        List<ProcessRecord> packageProcessAsUser = ProcessManagerService.get().getPackageProcessAsUser(callerPackage, userId);
    ...
    public void scheduleBroadcastReceiver(Intent intent, PendingResultData pendingResultData, int userId) throws RemoteException {
        List<ResolveInfo> resolves = PackageManagerService.get().queryBroadcastReceivers(intent, GET_META_DATA, null, userId);

        if (resolves.isEmpty()) {
            pendingResultData.build().finish();
            Logger.d(TAG, "scheduleBroadcastReceiver empty");
            return;
        }
    ...
    public AppConfig initProcess(String packageName, String processName, int userId) throws RemoteException {
        ProcessRecord processRecord = ProcessManagerService.get().startProcessLocked(packageName, processName, userId, -1, Binder.getCallingPid());
        if (processRecord == null)
            return null;
