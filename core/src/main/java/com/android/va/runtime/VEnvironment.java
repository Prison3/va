...
public class VEnvironment {
     ...
     */
    public static void ensureSandboxDataDirectories(String packageName, int userId) {
        File data = getDataDir(packageName, userId);
        FileUtils.mkdirs(data);
        FileUtils.mkdirs(getDataFilesDir(packageName, userId));
        FileUtils.mkdirs(new File(getDataFilesDir(packageName, userId), "mmkv"));
        FileUtils.mkdirs(getDataCacheDir(packageName, userId));
        FileUtils.mkdirs(new File(data, "code_cache"));
        FileUtils.mkdirs(getDataDatabasesDir(packageName, userId));
        FileUtils.mkdirs(new File(data, "shared_prefs"));
        FileUtils.mkdirs(new File(data, "no_backup"));
        FileUtils.mkdirs(new File(data, "app_xylog"));

        File de = getDeDataDir(packageName, userId);
        FileUtils.mkdirs(de);
        FileUtils.mkdirs(new File(de, "files"));
        FileUtils.mkdirs(new File(de, "cache"));
        FileUtils.mkdirs(new File(de, "code_cache"));
        FileUtils.mkdirs(new File(de, "shared_prefs"));
        FileUtils.mkdirs(new File(de, "databases"));
        FileUtils.mkdirs(new File(de, "no_backup"));
    }
    ...
    public static File getXSharedPreferences(String packageName, String prefFileName) {
       return new File(VEnvironment.getDataDir(packageName, VActivityThread.getUserId()), "shared_prefs/" + prefFileName + ".xml");
    }
}
