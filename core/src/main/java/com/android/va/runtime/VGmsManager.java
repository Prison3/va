...
public class VGmsManager {
    private static final String TAG = VGmsManager.class.getSimpleName();
    ...
    private static InstallResult installPackages(Set<String> list, int userId) {
        ...
        for (String packageName : list) {
            ...
            InstallResult installResult = VPackageManager.get().installPackageAsUser(packageName, userId);
    ...
    private static void uninstallPackages(Set<String> list, int userId) {
        for (String packageName : list) {
            VPackageManager.get().uninstallPackageAsUser(packageName, userId);
        }
    }
    ...
    public static boolean isInstalledGoogleService(int userId) {
        return VPackageManager.get().isInstalled(GMS_PKG, userId);
    }

    public static boolean uninstallGms(int userId) {
        uninstallGApps(userId);
        return !isInstalledGoogleService(userId);
    }
}
