 ...
 * {@hide}
 */
public abstract class IntentResolver<F extends VPackageInfo.IntentInfo, R extends Object> {
    ...
    public void addFilter(F f) {
        if (localLOGV) {
            Logger.v(TAG, "Adding filter: " + f);
            f.intentFilter.dump(new LogPrinter(Log.VERBOSE, TAG), "      ");
            Logger.v(TAG, "    Building Lookup Maps:");
        }
    ...
    void removeFilterInternal(F f) {
        if (localLOGV) {
            Logger.v(TAG, "Removing filter: " + f);
            f.intentFilter.dump(new LogPrinter(Log.VERBOSE, TAG), "      ");
            Logger.v(TAG, "    Cleaning Lookup Maps:");
        }
...
//        sortResults(resultList);
    ...
    public List<R> queryIntent(Intent intent, String resolvedType, boolean defaultOnly,
            ...
            if (slashpos > 0) {
                ...
                if (!baseType.equals("*")) {
                    if (resolvedType.length() != slashpos+2
                        ...
                        if (debug) Logger.v(TAG, "First type cut: " + Arrays.toString(firstTypeCut));
                        secondTypeCut = mWildTypeToFilter.get(baseType);
                        if (debug) Logger.v(TAG, "Second type cut: "
                    ...
                    if (debug) Logger.v(TAG, "Third type cut: " + Arrays.toString(thirdTypeCut));
                } else if (intent.getAction() != null) {
                    // The intent specified any type ({@literal *}/*).  This
                    // can be a whole heck of a lot of things, so as a first
                    // cut let's use the action instead.
                    firstTypeCut = mTypedActionToFilter.get(intent.getAction());
                    if (debug) Logger.v(TAG, "Typed Action list: " + Arrays.toString(firstTypeCut));
                }
            }
        }
        ...
        if (scheme != null) {
            ...
            if (debug) Logger.v(TAG, "Scheme list: " + Arrays.toString(schemeCut));
        }
        ...
        if (resolvedType == null && scheme == null && intent.getAction() != null) {
            ...
            if (debug) Logger.v(TAG, "Action list: " + Arrays.toString(firstTypeCut));
        }
...
//        sortResults(finalList);
        ...
        if (debug) {
            Logger.v(TAG, "Final result list:");
            for (int i=0; i<finalList.size(); i++) {
                Logger.v(TAG, "  " + finalList.get(i));
            }
        }
        return finalList;
    }
    ...
    private final int register_mime_types(F filter, String prefix) {
        ...
        while (i.hasNext()) {
            String name = i.next();
            num++;
            if (localLOGV) Logger.v(TAG, prefix + name);
...
//        final boolean excludingStopped = intent.isExcludingStopped();
        ...
        for (i=0; i<N && (filter=src[i]) != null; i++) {
            int match;
            if (debug) Logger.v(TAG, "Matching against filter " + filter);

//            if (excludingStopped && isFilterStopped(filter, userId)) {
//                if (debug) {
//                    Logger.v(TAG, "  Filter's target is stopped; skipping");
//                }
//                continue;
//            }
            ...
            if (packageName != null && !isPackageForFilter(packageName, filter)) {
                if (debug) {
                    Logger.v(TAG, "  Filter is not from package " + packageName + "; skipping");
                }
                continue;
            }
            ...
            if (match >= 0) {
                if (debug) Logger.v(TAG, "  Filter matched!  match=0x" +
                ...
                if (!defaultOnly || filter.intentFilter.hasCategory(Intent.CATEGORY_DEFAULT)) {
                    ...
                    if (debug) Logger.v(TAG, "    Created result: " + oneResult);
                    if (oneResult != null) {
                        dest.add(oneResult);
                    }
                } else {
        ...
        if (debug && hasNonDefaults) {
            if (dest.size() == 0) {
                Logger.v(TAG, "resolveIntent failed: found match, but none with CATEGORY_DEFAULT");
            } else if (dest.size() > 1) {
                Logger.v(TAG, "resolveIntent: multiple matches, only some with CATEGORY_DEFAULT");
            }
        }
    }
