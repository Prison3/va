...
inline static bool IsNativeMethod(char *art_method) {
    try {
        ...
        ALOGD("VNativeCore: Error checking native method flag, assuming not native");
        return false;
    }
}
...
void VJniHook::HookJniFun(JNIEnv *env, jobject java_method, void *new_fun, void **orig_fun, bool is_static) {
...
void VJniHook::HookJniFun(JNIEnv *env, const char *class_name, const char *method_name, const char *sign,
    ...
    // Clear any pending exceptions before getting method ID
    env->ExceptionClear();
    if (is_static) {
    ...
    if (!method || env->ExceptionCheck()) {
        // Method doesn't exist or exception occurred
        env->ExceptionClear();
        ALOGD("get method id fail: %s %s (method may not exist in this Android version)", class_name, method_name);
    ...
    *orig_fun = reinterpret_cast<void *>(artMethod[HookEnv.art_method_native_offset]);
    // Clear any pending exceptions before registering natives
    env->ExceptionClear();
    if (env->RegisterNatives(clazz, gMethods, 1) < 0) {
        // Registration failed, clear exception and return
        env->ExceptionClear();
        ALOGE("jni hook error. class：%s, method：%s (native method may not exist)", class_name, method_name);
        env->DeleteLocalRef(clazz);
        return;
    }
    // FastNative
...
void registerNative(JNIEnv *env) {
    jclass clazz = env->FindClass("com/android/va/runtime/VJniHook");
    JNINativeMethod gMethods[] = {
...
void VJniHook::InitJniHook(JNIEnv *env, int api_level) {
    registerNative(env);
...
//        ALOGD("art_method_size search:0x%x",value);
    ...
    HookEnv.method_utils_class = env->FindClass("com/android/va/base/MethodUtils");
