...
public class IXiaomiAttributionSourceProxy extends ClassInvocationStub {
    public static final String TAG = IXiaomiAttributionSourceProxy.class.getSimpleName();
    ...
    protected void inject(Object base, Object proxy) {
        ...
        if (XiaomiDeviceDetector.isXiaomiDevice()) {
            Logger.d(TAG, "IXiaomiAttributionSource proxy initialized for MIUI UID mismatch prevention on " + 
                    XiaomiDeviceDetector.getDeviceModel() + " (MIUI " + XiaomiDeviceDetector.getMiuiVersion() + ")");
        } else {
            Logger.d(TAG, "IXiaomiAttributionSource proxy initialized for MIUI UID mismatch prevention");
        }
    }
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            try {
                ...
                if (message != null && message.contains("Calling uid") && message.contains("doesn't match source uid")) {
                    Logger.w(TAG, "Xiaomi UID mismatch in AttributionSource constructor, creating safe fallback: " + message);
                    return AttributionSourceUtils.createSafeAttributionSource();
                }
                throw e;
            } catch (Exception e) {
                Logger.w(TAG, "Error in Xiaomi AttributionSource constructor hook: " + e.getMessage());
    ...
    public static class EnforceCallingUid extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            Logger.d(TAG, "Xiaomi enforceCallingUid bypassed to prevent UID mismatch crashes");
    ...
    public static class EnforceCallingUidAndPid extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            // Always allow on Xiaomi devices to prevent crashes
            Logger.d(TAG, "Xiaomi enforceCallingUidAndPid bypassed to prevent UID mismatch crashes");
            return null; // Indicate success
        }
    }
    ...
    public static class FromParcel extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            try {
                ...
                Logger.w(TAG, "Error in Xiaomi fromParcel hook: " + e.getMessage());
                // Create safe fallback for Xiaomi devices
                return AttributionSourceUtils.createSafeAttributionSource();
            }
        }
