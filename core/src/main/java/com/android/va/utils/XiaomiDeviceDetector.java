...
public class XiaomiDeviceDetector {
    private static final String TAG = XiaomiDeviceDetector.class.getSimpleName();
    ...
    public static boolean isXiaomiDevice() {
        ...
        try {
            ...
            if (isXiaomiManufacturer(manufacturer)) {
                ...
                Logger.d(TAG, "Detected Xiaomi device by manufacturer: " + manufacturer);
                return true;
            }
            ...
            if (isXiaomiManufacturer(brand)) {
                sIsXiaomiDevice = true;
                Logger.d(TAG, "Detected Xiaomi device by brand: " + brand);
                return true;
            }
            ...
            if (isXiaomiModel(model)) {
                sIsXiaomiDevice = true;
                Logger.d(TAG, "Detected Xiaomi device by model: " + model);
                return true;
            }
            ...
            if (isXiaomiModel(product)) {
                sIsXiaomiDevice = true;
                Logger.d(TAG, "Detected Xiaomi device by product: " + product);
                return true;
            }
            ...
            if (isXiaomiModel(device)) {
                sIsXiaomiDevice = true;
                Logger.d(TAG, "Detected Xiaomi device by device: " + device);
                return true;
            }
            ...
            if (isXiaomiFingerprint(fingerprint)) {
                sIsXiaomiDevice = true;
                Logger.d(TAG, "Detected Xiaomi device by fingerprint: " + fingerprint);
                return true;
            }
            ...
            if (checkXiaomiSystemProperties()) {
                sIsXiaomiDevice = true;
                Logger.d(TAG, "Detected Xiaomi device by system properties");
                return true;
            }
            
        } catch (Exception e) {
            Logger.w(TAG, "Error detecting Xiaomi device: " + e.getMessage());
        }
    ...
    public static String getMiuiVersion() {
        ...
        try {
            ...
            if (!TextUtils.isEmpty(sMiuiVersion)) {
                Logger.d(TAG, "Detected MIUI version: " + sMiuiVersion);
                return sMiuiVersion;
            }
            ...
            if (!TextUtils.isEmpty(sMiuiVersion)) {
                Logger.d(TAG, "Detected MIUI version code: " + sMiuiVersion);
                return sMiuiVersion;
            }
            ...
            if (!TextUtils.isEmpty(buildDesc) && containsMiuiVersion(buildDesc)) {
                ...
                Logger.d(TAG, "Detected MIUI version from build: " + sMiuiVersion);
                return sMiuiVersion;
            }
            
        } catch (Exception e) {
            Logger.w(TAG, "Error getting MIUI version: " + e.getMessage());
        }
    ...
    public static boolean isMiui12OrHigher() {
        ...
        try {
            ...
            Logger.w(TAG, "Error checking MIUI version: " + e.getMessage());
        }
    ...
    private static boolean checkXiaomiSystemProperties() {
        try {
            ...
            Logger.w(TAG, "Error checking Xiaomi system properties: " + e.getMessage());
        }
    ...
    private static String getSystemProperty(String key) {
        try {
            ...
            Logger.w(TAG, "Error getting system property " + key + ": " + e.getMessage());
            return null;
        }
    }
}
