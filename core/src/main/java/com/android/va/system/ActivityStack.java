...
public class ActivityStack {
    ...
    public int startActivityLocked(int userId, Intent intent, String resolvedType, IBinder resultTo, String resultWho, int requestCode, int flags, Bundle options) {
        ...
        ResolveInfo resolveInfo = PackageManagerService.get().resolveActivity(intent, GET_ACTIVITIES, resolvedType, userId);
    ...
    private Intent startActivityProcess(int userId, Intent intent, ActivityInfo
        ...
        ProcessRecord targetApp = ProcessManagerService.get().startProcessLocked(info.packageName, info.processName, userId, -1, Binder.getCallingPid());
        if (targetApp == null) {
