...
public class ProxyVpnService extends VpnService {
    ...
    public void onCreate() {
        super.onCreate();
        Logger.d(TAG, "ProxyVpnService created");
        createNotificationChannel();
    }

    @Override
    public int onStartCommand(Intent intent, int flags, int startId) {
        Logger.d(TAG, "ProxyVpnService started");
        ...
        try {
            ...
            Logger.e(TAG, "Critical error in onStartCommand: " + e.getMessage(), e);
    ...
    public void onDestroy() {
        super.onDestroy();
        stopVpn();
        Logger.d(TAG, "ProxyVpnService destroyed");
    }
    ...
    private Notification createNotification() {
        ...
        return builder
            .setContentTitle("Prison VPN Active")
    ...
    protected void establishVpn() {
        ...
        try {
            Logger.d(TAG, "Starting VPN establishment...");
            
            // Check timeout before proceeding
            if (System.currentTimeMillis() - startTime > TIMEOUT_MS) {
                Logger.w(TAG, "VPN establishment timeout, aborting");
                return;
            }
            ...
            if (System.currentTimeMillis() - startTime > TIMEOUT_MS) {
                Logger.w(TAG, "VPN establishment timeout before establish(), aborting");
                return;
            }
            ...
            if (mVpnInterface != null) {
                mIsEstablished = true;
                Logger.d(TAG, "VPN interface established successfully");
                
                // Start network monitoring and traffic handling in background
                startNetworkHandling();
            } else {
                Logger.e(TAG, "Failed to establish VPN interface - builder.establish() returned null");
            }
            
        } catch (Exception e) {
            Logger.e(TAG, "Error establishing VPN: " + e.getMessage());
            e.printStackTrace();
        ...
        // Final timeout check
        if (System.currentTimeMillis() - startTime > TIMEOUT_MS) {
            Logger.w(TAG, "VPN establishment took too long: " + (System.currentTimeMillis() - startTime) + "ms");
        }
    }
            ...
            public void run() {
                try {
                    while (mIsEstablished && mVpnInterface != null && !Thread.interrupted()) {
                        ...
                        // Log network status
                        Logger.d(TAG, "VPN interface active, monitoring network...");
                        ...
                        if (mVpnInterface != null) {
                            try {
                                ...
                                Logger.w(TAG, "VPN interface appears to be closed, re-establishing...");
                                reestablishVpn();
                                break;
                            }
                        }
                    ...
                    Logger.w(TAG, "Network monitoring interrupted: " + e.getMessage());
                } catch (Exception e) {
    ...
    private void reestablishVpn() {
        try {
            Logger.d(TAG, "Attempting to re-establish VPN connection");
            stopVpn();
            Thread.sleep(1000); // Wait a bit before re-establishing
            establishVpn();
        } catch (Exception e) {
            Logger.e(TAG, "Failed to re-establish VPN: " + e.getMessage());
        }
    }
    ...
    private void stopVpn() {
        ...
        if (mVpnInterface != null) {
            try {
                mVpnInterface.close();
                mVpnInterface = null;
                Logger.d(TAG, "VPN interface closed");
            } catch (Exception e) {
                Logger.w(TAG, "Error closing VPN interface: " + e.getMessage());
            }
        }
    }
    ...
    public void onRevoke() {
        super.onRevoke();
        Logger.w(TAG, "VPN service revoked by user");
        stopVpn();
    }
    ...
    public void onTrimMemory(int level) {
        ...
        if (level >= android.content.ComponentCallbacks2.TRIM_MEMORY_MODERATE) {
            Logger.d(TAG, "Memory trim requested, optimizing VPN service");
        }
    }
}
