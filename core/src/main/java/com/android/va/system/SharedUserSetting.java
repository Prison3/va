...
public final class SharedUserSetting implements Parcelable {
    public static final String TAG = SharedUserSetting.class.getSimpleName();
    ...
    public static void saveSharedUsers() {
        ...
        AtomicFile atomicFile = new AtomicFile(VEnvironment.getSharedUserConf());
        try {
    ...
    public static void loadSharedUsers() {
        Parcel parcel = Parcel.obtain();
        try {
            byte[] sharedUsersBytes = FileUtils.toByteArray(VEnvironment.getSharedUserConf());
            ...
            if (sharedUsersBytes.length < 100) { // Arbitrary threshold for old format
                Logger.w(TAG, "Detected old format SharedUserSetting data, clearing for fresh start");
                VEnvironment.getSharedUserConf().delete();
